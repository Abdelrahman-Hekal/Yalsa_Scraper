...
def scrape_yalsa(path):

    start = time.time()
    print('-'*75)
    print('Scraping yalsa.net ...')
    print('-'*75)
    ...
    if path == '':
        name = 'yalsa_data.xlsx'
        # getting the books under each category
        links = []
        nbooks, npages = 0, 0
        homepages = ['http://booklists.yalsa.net/directory/results']
        for homepage in homepages:
            driver.get(homepage)
            while True:           
                # scraping books urls
                titles = wait(driver, 10).until(EC.presence_of_all_elements_located((By.CSS_SELECTOR, "h2.title")))
                for title in titles:
                    try:
                        nbooks += 1
                        ...
                        print('The below error occurred during the scraping from yalsa.com, retrying ..')
                        print('-'*50)
                        print(err)
                        continue
                ...
                try:
                    ul = wait(driver, 2).until(EC.presence_of_element_located((By.CSS_SELECTOR, "ul.pages")))
                    buttons = wait(ul, 2).until(EC.presence_of_all_elements_located((By.TAG_NAME, "li")))
                    found = False
                    for button in buttons:
                        if "Next" in button.get_attribute('textContent'):
                            a = wait(button, 2).until(EC.presence_of_element_located((By.TAG_NAME, "a")))
        ...
        with open('yalsa_links.csv', 'w', newline='\n', encoding='utf-8') as f:
            writer = csv.writer(f)
    ...
    if path != '':
        ...
        df_links = pd.read_csv('yalsa_links.csv')
    ...
    for i, link in enumerate(links):
        ...
        try:
            ...
            try:
                title_link = link
                title = wait(driver, 5).until(EC.presence_of_element_located((By.TAG_NAME, "h1"))).get_attribute('textContent').strip()
            except Exception as err:
            ...
            author, author_link = '', ''
            try:
                div = wait(driver, 2).until(EC.presence_of_element_located((By.XPATH, "//div[@id='product-header']")))
            ...
            # other info
            genre, award, year, ISBN = '', '', '', ''
            try:
                ul = wait(driver, 5).until(EC.presence_of_element_located((By.CSS_SELECTOR, "ul.details.mute")))
                lis = wait(ul, 5).until(EC.presence_of_all_elements_located((By.TAG_NAME, "li")))
                for li in lis:
                    try:
                        tags = wait(li, 5).until(EC.presence_of_all_elements_located((By.TAG_NAME, "a")))
                        for a in tags:
                            url = a.get_attribute('href')
                            if 'genre' in url:
                                genre += a.get_attribute('textContent').strip() + ', '
                            elif 'year' in url:
    ...
    print('-'*75)
    print(f'yalsa.com scraping process completed successfully! Elapsed time {elapsed} mins')
    print('-'*75)
    driver.quit()
